...
public class AnimProcessor implements IAnimRefresh, IAnimOverScroll {
    ...
    public void dealPullDownRelease() {
        int touchSlop = cp.getTouchSlop();
        int headHeight = cp.getHeadHeight();
        int visibleHeadHeight = getVisibleHeadHeight();
        if (!cp.isPureScrollModeOn() && cp.enableRefresh() && visibleHeadHeight >= headHeight - touchSlop) {
            animHeadToRefresh();
        } else {
            animHeadBack(false);
        }
    ...
    public void dealPullUpRelease() {
        //leaffun: cp.getTouchSlop()导致issue#138
//        if (!cp.isPureScrollModeOn() && cp.enableLoadmore() && getVisibleFootHeight() >= cp.getBottomHeight() - cp.getTouchSlop()) {
        if (!cp.isPureScrollModeOn() && cp.enableLoadmore() && getVisibleFootHeight() >= cp.getBottomHeight()) {
            animBottomToLoad();
        } else {
